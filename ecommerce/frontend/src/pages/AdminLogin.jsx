...
export default function AdminLogin() {
  ...
  const openGoogleLogin = () => {
    window.open('/api/auth/google/start?role=admin', 'google-login', 'width=520,height=700')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ maxWidth: '420px', width: '100%' }}>
        {/* Logo & Header */}
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <div style={{ fontSize: '3em', marginBottom: '15px' }}>⚙️</div>
          <h1 style={{ fontSize: '2em', marginBottom: '5px', color: 'white', fontWeight: '700' }}>Admin Portal</h1>
          <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: '0.95em' }}>Manage platform, vendors & orders</p>
        </div>
        ...
        <form onSubmit={submit} style={{ background: 'white', borderRadius: '12px', padding: '30px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '0.95em', fontWeight: '600', marginBottom: '8px', color: '#333' }}>Email or Full Name</label>
            <input
              value={email}
              onChange={e => setEmail(e.target.value)}
              placeholder="admin@vendorhub.local"
              required
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '12px',
              background: loading ? '#ccc' : 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)',
              color: 'white',
              border: 'none',
              borderRadius: '8px',
              fontSize: '1em',
          ...
          <button
            type="button"
            onClick={() => navigate('/reset-password?role=admin')}
            style={{
              width: '100%',
              padding: '10px',
        ...
        <div style={{ textAlign: 'center', marginTop: '25px', color: 'white' }}>
          <p style={{ fontSize: '0.95em', marginBottom: '10px' }}>Don't have admin access?</p>
          <button
            onClick={() => navigate('/login?role=admin&mode=login')}
            style={{
              padding: '10px 20px',
